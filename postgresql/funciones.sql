...
update ejemplares set estado='l' where idmaterial=old.material and idejemplar=old.ejemplar; --actualizar estado
...
update ejemplares set estado='p' where idmaterial=new.material and idejemplar=new.ejemplar; --actualizar estado

select idres from reservas where nombre=new.nombre and material=new.material and activo='true' limit 1 INTO codigo;

IF(codigo is not null) THEN
select case when (fecha=current_date) then 'true' else 'false' end from reservas where idres=codigo INTO retiro;

	
if(retiro) then
		update reservas set retirado='true' where idres=codigo;
end if;
	update reservas set activo='false' where idres=codigo;
